/* FILE NAME: OBJ.C
 * PROGRAMMER: AH5
 * DATE: 14.06.2017
 * PURPOSE: Object handle module.
 */

#include <stdio.h>

#include "anim.h"

/* Materials table */
ah5MATERIAL AH5_RndMaterials[AH5_RES_TABLE_SIZE];
INT AH5_RndNumOfMaterials;

/* Textures table */
ah5TEXTURE AH5_RndTextures[AH5_RES_TABLE_SIZE];
INT AH5_RndNumOfTextures;

/* Shaders table */
ah5SHADER AH5_RndShaders[AH5_RES_TABLE_SIZE];
INT AH5_RndNumOfShaders;

/* Create object function.
 * ARGUMENTS:
 *   - created object:
 *       ah5OBJ *Obj;
 *   - number of primitives:
 *       INT NumOfP;
* RETURNS: None.
 */
VOID AH5_RndObjCreate( ah5OBJ *Obj, INT NumOfP )
{
  memset(Obj, 0, sizeof(ah5PRIM));
  if ((Obj->P = malloc(sizeof(ah5PRIM) * NumOfP)) == NULL)
    return;
  memset(Obj->P, 0, sizeof(ah5PRIM) * NumOfP);
  Obj->NumOfP = NumOfP;
} /* Ens of 'AH5_RndObjCreate' function */

/* Free object function.
 * ARGUMENTS:
 *   - deleted object:
 *       ah5OBJ *Obj;
 * RETURNS: None.
 */
VOID AH5_RndObjFree( ah5OBJ *Obj )
{
  INT i;

  for (i = 0; i < Obj->NumOfP; i++)
    AH5_RndPrimFree(&Obj->P[i]);
  free(Obj->P);
  memset(Obj, 0, sizeof(ah5PRIM));
} /* Ens of 'AH5_RndObjFree' function */

/* Draw object function.
 * ARGUMENTS:
 *   - drawing object:
 *       ah5OBJ *Obj;
 *   - transformation matrix:
 *       MATR M;
 * RETURNS: None.
 */
VOID AH5_RndObjDraw( ah5OBJ *Obj, MATR M )
{
  INT i;

  for (i = 0; i < Obj->NumOfP; i++)
    AH5_RndPrimDraw(&Obj->P[i], M);
} /* Ens of 'AH5_RndObjDraw' function */

/* Load object from "G3DM" file function.
 * ARGUMENTS:
 *   - drawing object:
 *       ah5OBJ *Obj;
 *   - file name:
 *       CHAR *FileName;
 * RETURNS:
 *   (BOOL) TRUE if success, FALSE otherwise.
 */
BOOL AH5_RndObjLoad( ah5OBJ *Obj, CHAR *FileName )
{
  FILE *F;
  DWORD Sign;
  INT NoofP = 0, NoofM = 0, NoofT = 0, i;

  memset(Obj, 0, sizeof(ah5OBJ));

  /* Open file */
  if ((F = fopen(FileName, "rb")) == NULL)
    return FALSE;

  /* Check signature */
  fread(&Sign, 4, 1, F);
  if (Sign != *(DWORD *)"G3DM")
  {
    fclose(F);
    return FALSE;
  }

  /* Read quantities */
  fread(&NoofP, 4, 1, F);
  fread(&NoofM, 4, 1, F);
  fread(&NoofT, 4, 1, F);

  if (AH5_RndNumOfMaterials + NoofM >= AH5_RES_TABLE_SIZE ||
      AH5_RndNumOfTextures + NoofT >= AH5_RES_TABLE_SIZE)
  {
    fclose(F);
    return FALSE;
  }

  /* Create object */
  AH5_RndObjCreate(Obj, NoofP);

  /* Load primitives */
  for (i = 0; i < NoofP; i++)
  {
    INT nv = 0, ni = 0, mtl_no = -1, size;
    ah5VERTEX *V;
    INT *I;

    fread(&nv, 4, 1, F);
    fread(&ni, 4, 1, F);
    fread(&mtl_no, 4, 1, F);
    size = sizeof(ah5VERTEX) * nv + sizeof(INT) * ni;
    if ((V = malloc(size)) == NULL)
    {
      fclose(F);
      AH5_RndObjFree(Obj);
      return FALSE;
    }
    fread(V, 1, size, F);
    I = (INT *)(V + nv);
    AH5_RndPrimCreate(&Obj->P[i], TRUE, V, nv, I, ni);
    free(V);
    if (mtl_no != -1)
      mtl_no += AH5_RndNumOfMaterials;
    Obj->P[i].MtlNo = mtl_no;
  }

  /* Load materials */
  fread(&AH5_RndMaterials[AH5_RndNumOfMaterials], sizeof(ah5MATERIAL), NoofM, F);
  for (i = 0; i < NoofM; i++)
  {
    INT j;
    ah5MATERIAL *mtl = &AH5_RndMaterials[AH5_RndNumOfMaterials + i];

    if (mtl->ShaderStr[0] != 0)
      mtl->Shader = AH5_RndShaderAdd(mtl->ShaderStr);

    for (j = 0; j < 8; j++)
      if (mtl->Tex[j] != -1)
        mtl->Tex[j] += AH5_RndNumOfTextures;
  }
  AH5_RndNumOfMaterials += NoofM;

  /* Load textures */
  for (i = 0; i < NoofT; i++)
  {
    CHAR Name[300];
    INT W, H, C;
    VOID *mem;

    fread(Name, 1, 300, F);
    fread(&W, 4, 1, F);
    fread(&H, 4, 1, F);
    fread(&C, 4, 1, F);
    if ((mem = malloc(W * H * C)) == NULL)
    {
      fclose(F);
      AH5_RndObjFree(Obj);
      return FALSE;
    }
    fread(mem, C, W * H, F);
    AH5_RndTextureAdd(Name, W, H, C, mem);
    free(mem);
  }
  AH5_RndNumOfTextures += NoofT;

  fclose(F);
  return TRUE;
} /* Ens of 'AH5_RndObjLoad' function */

/* END OF 'OBJ.C' FILE */
