...
VOID AH5_RndPrimCreate( ah5PRIM *Pr, BOOL IsTrimesh, ah5VERTEX *V, INT NumOfV, INT *I, INT NumOfI )
{
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(ah5VERTEX) * NumOfV, V, GL_STATIC_DRAW);
  ...
  glVertexAttribPointer(0, 3, GL_FLOAT, FALSE, sizeof(ah5VERTEX),
                        (VOID *)0); /* position */
  glVertexAttribPointer(1, 2, GL_FLOAT, FALSE, sizeof(ah5VERTEX),
                        (VOID *)sizeof(VEC)); /* texture coordinates */
  glVertexAttribPointer(2, 3, GL_FLOAT, FALSE, sizeof(ah5VERTEX),
                        (VOID *)(sizeof(VEC) + sizeof(VEC2))); /* normal */
  glVertexAttribPointer(3, 4, GL_FLOAT, FALSE, sizeof(ah5VERTEX),
...
} /* Ens of 'AH5_RndPrimCreate' function */
...
} /* Ens of 'AH5_RndPrimFree' function */
...
VOID AH5_RndPrimDraw( ah5PRIM *Pr, MATR M )
{
  ...
  glBindVertexArray(0);
} /* Ens of 'AH5_RndPrimDraw' function */
...
BOOL AH5_RndPrimLoad( ah5PRIM *Obj, CHAR *FileName )
{
  ...
  /* Memory allocation */
  size = sizeof(ah5VERTEX) * vn + sizeof(INT) * fn * 3;
  ...
  AH5_RndTriMeshEvalNormals(V, vn, I, fn);
  AH5_RndPrimCreate(Obj, TRUE, V, vn, I, fn);

  free(V);
  return TRUE;
} /* End of 'AH5_RndPrimLoad' function */
...
VOID AH5_RndTriMeshEvalNormals( ah5VERTEX *V, INT NumOfV, INT *I, INT NumOfI )
{
  ...
  {
    ah5VERTEX *p0 = &V[I[i]], *p1 = &V[I[i + 1]], *p2 = &V[I[i + 2]];
...
} /* End of 'AH5_RndTriMeshEvalNormals' function */
...
VOID AH5_RndGridEvalNormals( ah5VERTEX *V, INT N, INT M )
{
  ...
  for (i = 0; i < N * M; i++)
    V[i].N = VecNormalize(V[i].N);
} /* End of 'AH5_RndGridEvalNormals' function */
...
VOID AH5_RndPrimCreateSphere( ah5PRIM *Pr, VEC C, FLT R, INT N, INT M )
{
  ...
  INT
    i, j, k, v,
    *I,
    size = sizeof(ah5VERTEX) * N * M + sizeof(INT) * (M * 2 + 1) * (N - 1);
  DBL theta, phi;
  ...
  AH5_RndPrimCreate(Pr, FALSE, V, N * M, I, (M * 2 + 1) * (N - 1));

  free(V);
} /* Ens of 'AH5_RndPrimCreateSphere' function */
