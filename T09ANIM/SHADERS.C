 ...
 * RETURNS: None.
 */
static VOID AH5_RndShaderLog( CHAR *Stage, CHAR *Text )
{
  FILE *F;

  if ((F = fopen("AH5{SHAD}30.LOG", "a")) != NULL)
  {
    fprintf(F, "%s: %s\n", Stage, Text);
    fclose(F);
  }
} /* End of 'AH5_RndShaderLog' function */
 ...
 */
static CHAR * AH5_RndShaderLoadTextFile( CHAR *FileName )
{
  FILE *F;
  INT flen;
  CHAR *text;
  ...
  fclose(F);
  return text;
} /* End of 'AH5_RndShaderLoadTextFile' function */
...
UINT AH5_RndShaderLoad( CHAR *FileNamePrefix )
{
  ...
  for (i = 0; isok && i < NS; i++)
  {
    ...
    {
      AH5_RndShaderLog(suff[i], "Error create shader");
      isok = FALSE;
      break;
    }
    /* Load text file */
    sprintf(buf, "%s.%s", FileNamePrefix, suff[i]);
    if ((txt = AH5_RndShaderLoadTextFile(buf)) == NULL)
    {
      AH5_RndShaderLog(suff[i], "No file");
      isok = FALSE;
      break;
    }
  ...
  return prg;
} /* End of 'AH5_RndShaderLoad' function */
...
VOID AH5_RndShaderFree( UINT Prg )
{
  ...
  glDeleteProgram(Prg);
} /* End of 'AH5_RndShaderFree' function */
